...
- `provisional`: terrain-aware route altitude planning is specified in goal `0007` and `demo_drone_route_default_altitude_agl_v1`, but the code path and 3D elevated rendering are still todo.
...
| **Terrain-aware route altitude and 3D elevated drone paths** | `docs/goals/0007-terrain-aware-drone-route-altitude.md` | Adds `120 m AGL` default, AGL/MSL route profiles, degraded-terrain warnings, and Cesium 3D elevated route review. |
...
| **0007** | Terrain-aware route altitude profile, `120 m AGL` default, AGL/MSL/provenance fields, Cesium 3D elevated route display | Real drone control; certified obstacle clearance; per-waypoint altitude editing beyond TODO/stretch |
